 ...
 * @access public
 */
class NewsMunch_Customizer_Range_Control extends WP_Customize_Control {
	...
	/**
	 * NewsMunch_Customizer_Range_Control constructor.
	 *
 ...
 */
function newsmunch_sanitize_range_value( $input ) {
	if ( newsmunch_is_json( $input ) ) {
 ...
 * @param  string $string Input.
 */
function newsmunch_is_json( $string ) {
	return is_string( $string ) && is_array( json_decode( $string, true ) ) ? true : false;
}
